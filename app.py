...
# pool_pre_ping проверяет соединение перед использованием и переоткрывает
# его, если база «отвалилась» — на бесплатных хостингах это частая ситуация.
...
def init_database():
    ...
    На бесплатных хостингах база иногда «просыпается» на пару секунд позже,
    чем стартует сайт. Поэтому пробуем подключиться несколько раз с паузой —
    так сайт не падает на старте, а спокойно дожидается базу.
    """
    import time
    for attempt in range(1, 11):
        try:
            ...
            time.sleep(3)
    print("[init] ВНИМАНИЕ: не удалось подключиться к базе за 10 попыток.")
